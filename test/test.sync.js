...
describe( 'sync', function tests() {
	...
	it( 'should read a JSON5 file', function test() {
		var actual = read( file );
		assert.deepEqual( actual, data );
	});

	it( 'should read a JSON5 file using provided options', function test() {
		var actual;
	...
	it( 'should return an error if unable to parse file data as JSON5', function test() {
		var out = read( bad );
		assert.isTrue( out instanceof Error );
	});
