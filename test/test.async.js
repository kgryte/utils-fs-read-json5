...
describe( 'async', function tests() {
	...
	var file = path.join( __dirname, 'fixtures', 'config.json5' ),
		bad = path.join( __dirname, 'fixtures', 'bad.json5' );

	it( 'should export a function', function test() {
		expect( read ).to.be.a( 'function' );
	});

	it( 'should read a JSON5 file', function test( done ) {
		read( file, onRead );
	...
	it( 'should read a JSON5 file if provided an encoding option', function test( done ) {
	...
	it( 'should read a JSON5 file if provided an options object', function test( done ) {
	...
	it( 'should return an error if unable to parse file data as JSON5', function test( done ) {
		read( bad, onRead );
		function onRead( error ) {
			if ( error ) {
				assert.ok( error );
